 ...
 * ADMIN_ALLOWED_EMAILS and AUTH_SECRET as password; run pnpm e2e:ensure-staff once
...
test.describe("Admin E2E credentials", () => {
  test("staff reaches /admin after E2E credential sign-in", async ({
    ...
    if (result === "skip_no_ui") {
      test.skip(
        true,
        "E2E credentials UI not available (use /sign-in/e2e in development with ADMIN_ALLOWED_EMAILS + AUTH_SECRET). Run pnpm e2e:ensure-staff for Supabase user.",
      );
    }
    if (result === "skip_no_env") {
      test.skip(
        true,
        "Set ADMIN_ALLOWED_EMAILS and AUTH_SECRET in root .env.local for Playwright (loaded by playwright.config).",
      );
    }
  });
});
