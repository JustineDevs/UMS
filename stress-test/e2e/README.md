...
| Single file | `node stress-test/scripts/run-e2e.js stress-test/e2e/maharlika-commerce-architecture.spec.ts` |
...
Runs Stripe, PayPal, PayMongo, Maya, and COD (unless `E2E_STRESS_EXCLUDE_COD=1`) for `E2E_STRESS_ITERATIONS` each. Requires **storefront customer auth** for add-to-bag: save Playwright `storageState` after a real Google sign-in and set `PLAYWRIGHT_STOREFRONT_STORAGE_STATE` to that JSON path.
...
PSP sandbox keys match the existing PSP specs (`E2E_STRIPE_API_KEY`, `E2E_PAYPAL_CLIENT_ID` + `E2E_PAYPAL_CLIENT_SECRET`, etc.).
