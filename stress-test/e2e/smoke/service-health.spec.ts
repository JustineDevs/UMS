/**
 * @smoke
 * Service health smoke test — asserts the API, Medusa, and unified web return 200 before
...
test.describe("@smoke Service health checks", () => {
  ...
  test("apps/web GET /api/health returns 200 with status ok", async ({ request }) => {
    const res = await request.get(`${STOREFRONT_URL}/api/health`, {
      failOnStatusCode: false,
    });
    expect(
      res.status(),
      `apps/web :3000/api/health → expected 200, got ${res.status()}`,
    ).toBe(200);
  ...
  test("apps/web admin route returns 200 or 3xx", async ({ request }) => {
    const res = await request.get(`${WEB_URL}/admin`, {
      failOnStatusCode: false,
      maxRedirects: 0,
    });
    ...
    expect(
      ...
      `apps/web /admin → expected 200-3xx, got ${status}`,
    ).toBeTruthy();
  });
});
