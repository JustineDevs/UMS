...
import { UVS_SOURCE_IMAGES } from "./source-images";
...
test("core definitions preserve source markup and property bindings", () => {
  ...
  assert.equal(heading?.image, UVS_SOURCE_IMAGES["html/heading"]);
  assert.ok(UVS_CORE_DEFINITIONS.every((item) => item.source.includes("ported-vvveb/")));
});
...
test("captured source registrations retain original markup, matching, and property keys", () => {
  for (const source of UVS_SOURCE_CAPTURE) {
    ...
    if (typeof source.html === "string") assert.equal(definition.markup, source.html, source.type);
    ...
    if (UVS_SOURCE_IMAGES[source.type]) assert.equal(definition.image, UVS_SOURCE_IMAGES[source.type], source.type);
...
test("ecommerce definitions preserve the imported source module objects", () => {
  ...
  for (const [type, exportName] of Object.entries(names)) {
    ...
    if (source.html !== undefined) assert.equal(definition.markup, source.html.trim(), type);
    ...
    assert.equal(definition.image, source.image, type);
  }
});
...
test("every registered component family resolves to an editable definition", () => {
  for (const family of UVS_CONCRETE_COMPONENTS) {
    ...
    const captured = UVS_SOURCE_CAPTURE.find((source) => source.type === family.type);
    const ecommerce = Object.values(UVS_ECOMMERCE_CAPTURE).find((source) => source.name === definition.name && source.html?.trim() === "");
    if (!captured && !ecommerce) assert.ok(definition.markup.length > 0, family.type);
    ...
    assert.ok(!definition.markup.includes('data-uvs-source='), `generated fallback markup: ${family.type}`);
  }
});
