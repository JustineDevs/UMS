...
test("style manager applies Vvveb theme attributes and CSS states", () => {
  const manager = new StyleManager();
  manager.setTheme("dark");
  manager.setState("hover");
  manager.setStyle(".card", "color", "white");
  assert.match(manager.cssText, /\[data-bs-theme="dark"\] \.card:hover/);

  manager.themeMode = "media";
  manager.setTheme("light");
  manager.setState("");
  manager.setStyle(".card", "color", "black");
  assert.match(manager.cssText, /@media \(prefers-color-scheme: light\)/);
});
