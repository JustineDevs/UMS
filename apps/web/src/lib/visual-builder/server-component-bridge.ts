...
const editorSaveRequestSchema = z.object({ component: z.string().min(1).max(100), id: z.string().min(1).max(100), expectedVersion: z.number().int().positive(), fields: z.array(fieldSchema).max(100) }).strict();
...
function sanitizeEditorFields(fields: readonly z.infer<typeof fieldSchema>[]): z.infer<typeof fieldSchema>[] {
