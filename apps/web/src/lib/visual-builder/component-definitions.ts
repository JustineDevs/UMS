...
const htmlSource = "ported-vvveb/components-html.js";
const baseSource = "ported-vvveb/components-common.js";
...
const UVS_HTML_DEFINITIONS: readonly VisualComponentDefinition[] = [
...
const sourceAlignedDefinitions = allDefinitions.map((definition) => {
  ...
  if (!captured) return definition.image ? definition : { ...definition, image: UVS_SOURCE_IMAGES[definition.type] };
  const capturedMarkup = typeof captured.html === "string" ? captured.html : definition.markup;
  const capturedProps = capturedProperties(definition.source, captured.properties);
  return {
    ...definition,
    image: definition.image ?? UVS_SOURCE_IMAGES[definition.type],
