...
class FakeQuery {
  ...
  then<TResult1 = unknown, TResult2 = never>(
    onfulfilled?: ((_value: { data: null; error: null }) => TResult1 | PromiseLike<TResult1>) | null,
  ...
  return {
    from(table: string) {
      ...
      const query = {
        ...
        then: (resolve: (_value: { data: Array<Record<string, unknown>>; error: null }) => unknown) =>
          Promise.resolve({
...
describe("negative API security coverage", () => {
  ...
  it("caps form export reads even when the caller requests an unbounded limit", async () => {
    ...
    const query = {
      ...
      gte: () => query,
      lte: () => query,
      then: (resolve: (_value: { data: never[]; count: number; error: null }) => unknown) =>
