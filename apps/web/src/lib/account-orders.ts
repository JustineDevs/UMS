...
/** Shopper-facing KPIs from the complete customer-scoped order scan. */
...
async function fetchWorkerCustomerOrders(): Promise<{
  orders: AccountOrder[];
  error: string | null;
} | null> {
  ...
  try {
    ...
    if (!accessToken)
      return { orders: [], error: "Sign in to view your order history." };
    const response = await fetch(
      `${base}/store/customers/me/orders?limit=100`,
      {
        headers: {
        ...
        return [
          {
            ...
            total: Number.isFinite(totalMinor)
              ? medusaMinorToMajor(totalMinor, currency)
              : 0,
            currency,
...
export async function fetchWorkerCustomerOrderDetail(
  ...
  | { order: AccountOrderDetail; error: null }
...
> {
  ...
  try {
    ...
    const accessToken = data.session?.access_token?.trim();
    if (!accessToken)
      return { order: null, error: "Sign in to view this order." };
    const response = await fetch(
    ...
    const minor = (value: unknown): number | undefined => {
      ...
      return Number.isFinite(parsed)
        ? medusaMinorToMajor(parsed, currency)
        : undefined;
    };
          ...
          return [
            {
              ...
              variant:
                item.variant && typeof item.variant === "object"
                  ? {
                      sku:
                        typeof (item.variant as Record<string, unknown>).sku ===
                        "string"
      ...
      order: {
        ...
        shipping_address:
          address && typeof address === "object"
            ? (address as Record<string, string | null>)
            : null,
        items,
        fulfillments: Array.isArray(raw.fulfillments)
          ? (raw.fulfillments as AccountOrderDetail["fulfillments"])
          : [],
      },
      error: null,
    };
...
/** Allows migrated orders only when no customer id is attached and email matches exactly. */
...
/** Maps provider status aliases into the shopper-facing order buckets. */
export function accountOrderMatchesStatusFilter(
  status: string,
  filter: string,
): boolean {
...
export function countAccountOrderItems(
  items: AdminOrderListRow["items"],
): number {
...
export function buildAccountOrdersQuery(
  ...
  const params = new URLSearchParams({
    fields:
      "id,email,customer_id,display_id,status,total,currency_code,created_at,*items",
    limit: "100",
    offset: String(offset),
    order: "-created_at",
  });
  params.set(
    customerId ? "customer_id" : "email",
    customerId ?? email.trim().toLowerCase(),
  );
  return `/admin/orders?${params.toString()}`;
}

/** Fetches orders from the authenticated Worker customer-order contract. */
export async function fetchCustomerOrders(
  ...
  return {
    orders: [],
    error: "Worker API is not configured for order history.",
  };
}
