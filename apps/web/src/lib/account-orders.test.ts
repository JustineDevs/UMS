...
test("account order filters normalize provider status aliases", () => {
  assert.equal(accountOrderMatchesStatusFilter("canceled", "cancelled"), true);
  assert.equal(
    accountOrderMatchesStatusFilter("partially_shipped", "shipped"),
    true,
  );
  assert.equal(
    accountOrderMatchesStatusFilter("requires-action", "pending_payment"),
    true,
  );
  assert.equal(
    accountOrderMatchesStatusFilter("return_requested", "returned"),
    true,
  );
  assert.equal(
    accountOrderMatchesStatusFilter("delivered", "cancelled"),
    false,
  );
});
...
test("account stats aggregate a single currency", () => {
  assert.deepEqual(
    computeAccountOrderStats([order("PHP", 100), order("PHP", 50)]),
    {
      orderCount: 2,
      lifetimeSpend: 150,
...
test("account email normalization trims and lowercases before lookup", () => {
  assert.equal(
    normalizeAccountEmail("  Buyer@Example.COM "),
    "buyer@example.com",
  );
});
...
test("account stats do not add unlike currencies", () => {
  assert.deepEqual(
    computeAccountOrderStats([order("PHP", 100), order("USD", 50)]),
    {
      orderCount: 2,
      lifetimeSpend: null,
...
test("account order item count reports units instead of line count", () => {
  ...
  assert.equal(
    countAccountOrderItems([{ quantity: "3" }, { quantity: -2 }]),
    0,
  );
});
...
test("account order detail permits only canonical or legacy ownership", () => {
  assert.equal(
    accountOrderMatchesDetail(
      { customer_id: null, email: "buyer@example.com" },
      "cus_123",
      "buyer@example.com",
    ),
    true,
  ...
  assert.equal(
    accountOrderMatchesDetail(
      { customer_id: null, email: "other@example.com" },
      "cus_123",
      "buyer@example.com",
    ),
    false,
  ...
  assert.equal(
    accountOrderMatchesDetail(
      { customer_id: "cus_other", email: "buyer@example.com" },
      "cus_123",
      "buyer@example.com",
    ),
    false,
  ...
  assert.equal(
    accountOrderMatchesDetail(
      { customer_id: "cus_123", email: "buyer@example.com" },
      "cus_123",
      "buyer@example.com",
    ),
    true,
...
test("account order view distinguishes signed out, loading, unavailable, empty, and ready", () => {
  ...
  assert.equal(
    getAccountOrderViewState({ ...base, authenticated: false }),
    "signed_out",
  );
  assert.equal(getAccountOrderViewState({ ...base, loading: true }), "loading");
  assert.equal(
    getAccountOrderViewState({ ...base, error: "unavailable" }),
    "error",
  );
