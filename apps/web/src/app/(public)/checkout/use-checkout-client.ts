...
export function useCheckoutClient({
  ...
  const [medusaPricePreview, setMedusaPricePreview] =
    useState<CheckoutTotalsPreview | null>(null);
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (paymentMethod === "COD") {
        ...
        if (!codRes.ok) {
          const codJson = (await codRes.json().catch(() => ({}))) as {
            error?: string;
            missingFields?: string[];
          };
          const hint =
      ...
      if (
        ...
        if (typeof data.trackingPageUrl !== "string") {
          throw new Error(
            "Could not create a secure order-tracking link. Please try again.",
          );
        }
      ...
      if (!checkoutCorrelationId) {
        const regRes = await fetch("/api/payments/checkout-intents", {
          ...
          body: JSON.stringify({
            ...
            providerPaymentId:
              providerPaymentId ??
              (paymentMethod === "PAYPAL" ? paypalOrderId : undefined),
          }),
        });
        if (!regRes.ok) {
        ...
        if (typeof regJson.correlationId !== "string") {
          throw new Error(
            "Could not register a durable payment attempt. Please try again.",
          );
        }
  ...
  async function finalizeCheckoutAttempt(active: {
    ...
    try {
      ...
      const finalizeJson = finalizeRes.ok
        ? ((await finalizeRes.json().catch(() => ({}))) as {
            error?: string;
            redirectUrl?: string;
          })
        : {};
