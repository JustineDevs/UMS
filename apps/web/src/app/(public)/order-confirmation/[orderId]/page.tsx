...
async function fetchOrderData(
  ...
  if (capability.id.startsWith("order_") && process.env.API_URL?.trim()) {
    const r = await fetchWorkerTrackByToken(token, { includeConfirmation: true });
    return {
      ok: r.ok,
      status: r.status,
  ...
  if (capability.id.startsWith("cart_")) return { ok: false, data: null, status: 503 };
...
export default async function OrderConfirmationPage({
  ...
  const { ok, data, status, correlationId } = await fetchOrderData(
    capability,
    encodedId.slice(4),
  );
