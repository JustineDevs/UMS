...
export async function GET(request: Request) {
  ...
  const settlementQuery = sup.client
    ...
    .limit(500);
  if (settlementProvider !== "all") settlementQuery.eq("provider", settlementProvider);

  const [metrics, recentAttempts, reconciliationArtifacts, settlementResult] = await Promise.all([
    getPaymentPlatformMetrics(sup.client, organization.id),
    listRecentPaymentAttempts(sup.client, 500, organization.id),
    listPaymentProviderArtifacts(sup.client, {
  ...
  const { data: settlementRows } = settlementResult;
