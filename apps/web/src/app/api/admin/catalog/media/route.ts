...
import {
  fetchWorkerCatalogMediaForAdmin,
  uploadWorkerCatalogMediaForAdmin,
...
export async function GET(req: NextRequest) {
  ...
  const workerMedia = await fetchWorkerCatalogMediaForAdmin({
...
async function post(req: NextRequest) {
  ...
  if (process.env.API_URL?.trim()) {
    ...
    const response = await uploadWorkerCatalogMediaForAdmin(form, idempotencyKey);
    if (!response) return correlatedJson(cid, { error: "Commerce Worker is unavailable" }, { status: 503 });
    const payload = (await response.json().catch(() => ({}))) as Record<string, unknown>;
    if (!response.ok) return correlatedJson(cid, payload, { status: response.status });
