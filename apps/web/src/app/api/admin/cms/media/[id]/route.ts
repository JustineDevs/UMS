...
import { deleteWorkerCatalogMediaForAdmin } from "@/lib/worker-admin-bridge";
...
async function deleteHandler(_req: NextRequest, ctx: RouteCtx) {
  ...
  if (process.env.API_URL?.trim()) {
    ...
    const response = await deleteWorkerCatalogMediaForAdmin({ mediaId: id, idempotencyKey });
    if (!response) return correlatedJson(cid, { error: "Commerce Worker is unavailable" }, { status: 503 });
    const payload = (await response.json().catch(() => ({}))) as Record<string, unknown>;
    if (!response.ok) {
      const code = payload.error === "media_in_use" ? "MEDIA_IN_USE" : response.status === 403 ? "FORBIDDEN" : "MEDIA_DELETE_FAILED";
      return correlatedJson(cid, { error: typeof payload.error === "string" ? payload.error : "Media deletion failed", code }, { status: response.status });
    }
    return correlatedJson(cid, payload, { status: response.status });
  }
  const canContentWrite = staffSessionAllows(session, "content:write");
  const canCatalogWrite = staffSessionAllows(session, "catalog:write");
  const sup = adminSupabaseOr503(cid);
