...
import { fetchWorkerStorefrontMetadataForAdmin, saveWorkerStorefrontMetadataForAdmin } from "@/lib/worker-admin-bridge";
...
export async function GET(request: Request) {
  ...
  const response = await fetchWorkerStorefrontMetadataForAdmin();
...
export async function PUT(request: Request) {
  ...
  const response = await saveWorkerStorefrontMetadataForAdmin(body, idempotencyKey);
  return response ?? new Response(JSON.stringify({ error: "Worker backend is unavailable", requestId: correlationId }), { status: 503, headers: { "Content-Type": "application/json", "x-request-id": correlationId } });
}
