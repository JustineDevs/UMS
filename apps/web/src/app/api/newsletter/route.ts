...
async function handler(request: Request): Promise<Response> {
  ...
  try {
    const upstream = await fetch(`${apiUrl}/store/newsletter`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: await request.clone().text(), cache: "no-store" });
