...
import { posCommerceCommitSaleResponseSchema } from "@/lib/admin-api-contracts";
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${base}/api/admin/pos/sales`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "Idempotency-Key": key }, body: await request.text(), cache: "no-store" });
  ...
  const parsed = posCommerceCommitSaleResponseSchema.safeParse(payload);
  if (!parsed.success) return Response.json({ error: "POS sale returned an invalid response" }, { status: 502 });
