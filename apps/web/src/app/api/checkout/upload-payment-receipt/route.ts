...
async function handlePOST(req: NextRequest) {
  ...
  const organizationId = resolvePaymentReceiptOrganizationId({
    paymentAttemptOrganizationId:
      typeof paymentAttempt?.organization_id === "string"
        ? paymentAttempt.organization_id
        : null,
    configuredOrganizationId: process.env.DEFAULT_ORGANIZATION_ID,
  });
  if (!organizationId) {
    return NextResponse.json(
      { error: "Store organization is not configured" },
      { status: 503 },
    );
  }
  ...
  if (!matchesReceiptSignature(mimeType, buffer)) {
    return NextResponse.json(
      { error: "Receipt contents do not match the declared file type" },
      { status: 400 },
    );
  }
  ...
  if (uploadError) {
    console.error(
      "[upload-payment-receipt] storage upload error:",
      uploadError,
    );
    return NextResponse.json(
  ...
  if (dbError) {
    ...
    return NextResponse.json(
      { error: "Failed to record receipt. Contact support." },
      { status: 503 },
    );
  }
  ...
  if (signedUrlError || !signedUrl?.signedUrl) {
    ...
    return NextResponse.json(
      { error: "Receipt uploaded; preview is temporarily unavailable." },
      { status: 202 },
    );
  }
