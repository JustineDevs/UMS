...
async function handler(request: Request): Promise<Response> {
  ...
  const value = bounded.valid && bounded.value && typeof bounded.value === "object" ? bounded.value as Record<string, unknown> : {};
  const cartId = typeof value.cartId === "string" ? value.cartId.trim().slice(0, 120) : "";
  const code = typeof value.code === "string" ? value.code.trim().toUpperCase().slice(0, 64) : "";
  if (!cartId || !code) return Response.json({ ok: false, error: "cartId and code are required", code: "MISSING_PARAMS" }, { status: 400 });
  ...
  try {
    ...
    try { parsed = JSON.parse(payload) as Record<string, unknown>; } catch { parsed = { error: "Invalid promotion response" }; }
    if (!upstream.ok) {
    ...
    return Response.json({ ok: true, discountAmount: parsed.discountAmount ?? 0 });
