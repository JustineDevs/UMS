...
import { fetchWorkerOrderDetailForAdmin } from "@/lib/worker-admin-bridge";
...
export default async function OrderDetailPage({
  ...
  const result = await fetchWorkerOrderDetailForAdmin(orderId);
  const order = result?.order ?? null;
  const shipments: ShipmentRow[] = result?.order.shipments ?? [];
