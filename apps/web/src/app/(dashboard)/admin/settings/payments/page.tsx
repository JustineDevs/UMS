...
const SUCCESSFUL_PAYMENT_STATUSES = new Set(["succeeded", "paid", "authorized", "captured"]);
...
export default async function PaymentSettingsPage({
  ...
  const successful = attempts.filter((attempt) => SUCCESSFUL_PAYMENT_STATUSES.has(attempt.status));
  ...
  const transactionData: FinanceTransactionPoint[] = Array.from({ length: 7 }, (_, index) => {
    ...
    const income = dayAttempts
      .filter((attempt) => SUCCESSFUL_PAYMENT_STATUSES.has(attempt.status))
