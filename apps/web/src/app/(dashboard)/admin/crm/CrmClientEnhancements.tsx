...
const ACTION_GROUPS: { label: string; actions: [ActivePanel, string][] }[] = [
  {
    label: "Customer insights",
    actions: [["segments", "Customer segments"], ["clv", "Customer lifetime value"], ["roster", "Customer roster"]],
  },
  { label: "Connections", actions: [["connection", "Integrations"]] },
  {
    label: "Sync health",
    actions: [["record", "Synced person or deal"], ["records", "Synced people and deals"], ["mappings", "Customer mappings"]],
  },
];
...
export function CrmClientEnhancements({
  ...
  const fetchNango = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error ?? "Unable to load CRM connections");
      }
      const body = await res.json().catch(() => ({})) as { data?: CrmNangoConnection[]; integrations?: CrmNangoIntegration[] };
      ...
      onEvent: async (event) => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(body.error ?? "Unable to save CRM connection");
          }
  ...
  async function connectNango() {
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({})) as { error?: string };
        throw new Error(body.error ?? "Unable to start secure CRM connection");
      }
      const body = await response.json() as { data?: { session_token?: string } };
      if (!body.data?.session_token) throw new Error("Unable to start secure CRM connection");
  ...
  async function disconnectNango(connection: CrmNangoConnection) {
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => ({})) as { error?: string };
        throw new Error(body.error ?? "Unable to disconnect CRM provider");
      }
      ...
      <Dialog open={activePanel !== null} onOpenChange={(open) => !open && setActivePanel(null)}>
        <DialogContent className="max-h-[min(88dvh,760px)] overflow-y-auto sm:max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2 border-b pb-3">
            {ACTION_GROUPS.map((group) => <Button key={group.label} type="button" size="sm" variant={group.actions.some(([panel]) => panel === activePanel) ? "default" : "outline"} onClick={() => setActivePanel(group.actions[0][0])}>{group.label}</Button>)}
          </div>
