...
export function StorefrontHomeVisualEditor({
  ...
  const canWrite = devMode || process.env.NODE_ENV === "development" || session?.user?.role === "admin" || staffHasPermission(
  ...
  useEffect(() => {
    ...
    void Promise.all([
      fetch("/api/admin/storefront-home", { signal: controller.signal }),
  ...
  const save = async (currentBlocks = blocks) => {
    if (!payload || !canWrite) {
      setError(!canWrite ? "You do not have permission to save the homepage." : "Homepage is still loading.");
      return;
    }
    setError(null);
    setSaved(false);
