...
export function CmsSiteMapPanel() {
  ...
  const load = useCallback((signal?: AbortSignal) => {
    ...
    setLoading(true);
    Promise.all([
      fetch("/api/admin/cms/pages", { signal }).then(async (r) => {
        const j = (await r.json()) as { data?: PageRow[]; error?: string };
        if (!r.ok) throw new Error(j.error ?? "pages");
        return j.data ?? [];
      }),
      fetch("/api/admin/cms/navigation", { signal }).then(async (r) => {
      ...
      .catch((e: unknown) => {
        if (!signal?.aborted) setErr(e instanceof Error ? e.message : "Load failed");
      })
      .finally(() => {
