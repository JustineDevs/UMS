...
function normalizeFooterLinks(value: unknown): FooterLinkRow[] {
  if (!Array.isArray(value)) return [];
  const links: FooterLinkRow[] = [];
  for (const item of value as { href?: string; label?: string }[]) {
    if (!item || typeof item.href !== "string" || typeof item.label !== "string") continue;
    links.push({ href: item.href, label: item.label, lid: newDragId() });
  }
  return links;
}
...
function normalizeNavToEditor(raw: unknown): EditorNav {
  ...
  const headerLinks: HeaderRow[] = [];
  if (Array.isArray(o.headerLinks)) {
    for (const item of o.headerLinks as {
      href?: string;
      label?: string;
      badge?: string;
      iconKey?: string;
      ...
      headerLinks.push({
        ...
        children: normalizeFooterLinks(item.children),
      });
    }
  }
  const normalizedHeaderLinks = Array.isArray(o.headerLinks) ? headerLinks : e.headerLinks;
  const headerLinksMobile = Array.isArray(o.headerLinksMobile)
    ? normalizeFooterLinks(o.headerLinksMobile)
    : e.headerLinksMobile;
  const footerBottomLinks = Array.isArray(o.footerBottomLinks)
    ? normalizeFooterLinks(o.footerBottomLinks)
    : e.footerBottomLinks;
  const footerColumns: FooterColRow[] = [];
  if (Array.isArray(o.footerColumns)) {
    for (const col of o.footerColumns as { title?: string; links?: unknown }[]) {
      footerColumns.push({
        cid: newDragId(),
        title: typeof col.title === "string" ? col.title : "Links",
        links: normalizeFooterLinks(col.links),
      });
    }
  }
  const normalizedFooterColumns = Array.isArray(o.footerColumns) ? footerColumns : e.footerColumns;
  const socialLinks: SocialRow[] = [];
  if (Array.isArray(o.socialLinks)) {
    for (const item of o.socialLinks as { href?: string; label?: string; network?: string }[]) {
      if (!item || typeof item.href !== "string" || typeof item.label !== "string") continue;
      socialLinks.push({
        href: item.href,
        label: item.label,
  ...
  return {
    ...
    headerLinksMobile,
    footerBottomLinks,
    footerColumns: normalizedFooterColumns,
    socialLinks,
  };
}
