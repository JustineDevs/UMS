...
export function AddToCartSection({
  ...
  const { types, finishes } = useMemo(() => {
    ...
    for (const variant of product.variants) {
      if (variant.type) typeSet.add(variant.type);
      if (variant.finish) finishSet.add(variant.finish);
    }
    return { types: [...typeSet].sort(), finishes: [...finishSet].sort() };
  }, [product.variants]);
  ...
  const finishesForType = useMemo(
    () => {
      ...
      for (const variant of product.variants) {
        if (variant.type === type && isVariantSellable(variant) && isVariantAvailable(variant.id)) {
