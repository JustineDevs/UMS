...
const ICON_PATHS: Record<"search" | "copy" | "route" | "alert" | "truck", React.ReactNode> = {
...
function Icon({ name, className }: { name: "search" | "copy" | "route" | "alert" | "truck"; className?: string }) {
  return <svg aria-hidden="true" className={cn("size-4", className)} fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" viewBox="0 0 24 24">{ICON_PATHS[name]}</svg>;
}
