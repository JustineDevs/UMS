...
export const recentOrdersColumns: ColumnDef<OrderRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const orderHref = row.original.backendId ? `/admin/orders/${encodeURIComponent(row.original.backendId)}` : "/admin/orders";
      const customer = row.original.customer.trim();
      const canEmailCustomer = customer.includes("@");

      return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="flex w-full justify-end">
            <Button aria-label={`Open actions for order ${row.original.id}`} size="icon-sm" variant="ghost">
              <MoreHorizontal />
            </Button>
          </div>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end" className="w-40">
          ...
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem asChild={canEmailCustomer} disabled={!canEmailCustomer}>
              {canEmailCustomer ? <a href={`mailto:${customer}`}>Contact customer</a> : <span>Contact customer</span>}
            </DropdownMenuItem>
            <DropdownMenuItem onSelect={() => void navigator.clipboard?.writeText(row.original.backendId ?? row.original.id)}>
              Copy order ID
            </DropdownMenuItem>
          </DropdownMenuGroup>
        </DropdownMenuContent>
