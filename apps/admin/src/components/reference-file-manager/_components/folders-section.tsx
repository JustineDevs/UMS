...
import { Clock, Copy, Folder, MoreVertical, ExternalLink } from "lucide-react";
...
export function FoldersSection({ folders }: FoldersSectionProps) {
  const folderUrl = (id: string) => sanitizeSameOriginUrl(
    `/admin/catalog/media?folder=${encodeURIComponent(id)}`,
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuGroup>
                        <DropdownMenuItem onSelect={() => {
                          const url = folderUrl(folder.id);
                          if (url) window.location.assign(url);
                        }}><ExternalLink />Open folder</DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => {
                          const url = folderUrl(folder.id);
                          if (url) void navigator.clipboard?.writeText(url);
                        }}><Copy />Copy share link</DropdownMenuItem>
                      </DropdownMenuGroup>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </CardAction>
