...
export type AdminSession = { user: { id?: string; email?: string; name?: string | null; image?: string | null; role?: string; permissions?: string[] }; expires: string };
type State = { data: AdminSession | null; status: "loading" | "authenticated" | "unauthenticated"; update: (patch?: { name?: string | null }) => Promise<void> };
...
export async function signIn(provider: "google", options?: { callbackUrl?: string }) { const callback = new URL("/api/auth/callback", window.location.origin); if (options?.callbackUrl?.startsWith("/")) callback.searchParams.set("next", options.callbackUrl); const { error } = await createSupabaseBrowserClient().auth.signInWithOAuth({ provider, options: { redirectTo: callback.toString() } }); if (error) throw error; }
export async function signOut(options?: { callbackUrl?: string }) { await createSupabaseBrowserClient().auth.signOut(); window.location.assign(options?.callbackUrl ?? "/sign-in"); }
