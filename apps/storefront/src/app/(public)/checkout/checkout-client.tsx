    ...
    return (
      <main className="storefront-page-shell motion-surface max-w-4xl">
        ...
        <p className="font-body text-on-surface-variant mb-6 max-w-lg">
          Sign in for a faster checkout with saved address and order history. Or continue as a guest
          with card payment.
        </p>
        ...
        <p className="mt-4 text-xs text-on-surface-variant">
          Guest checkout supports card payments only. Cash on delivery requires a saved delivery
          profile.
        </p>
      </main>
    );
      ...
      <main className="storefront-page-shell motion-surface max-w-4xl">
        ...
        >
          <p className="font-body text-on-surface mb-3">
            Checkout is temporarily unavailable. Your bag is saved. You can try again in a moment or return
            to your bag.
          </p>
    ...
    <main className="storefront-page-shell motion-surface max-w-4xl">
      ...
      <nav aria-label="Checkout progress" className="mb-8">
        <ol className="flex items-center gap-0">
          {(["Address", "Shipping", "Payment", "Review"] as const).map((step, i, arr) => {
            const stepIndex = i + 1;
            const activeStep =
              profileGate !== "complete" ? 1 :
              !selectedShippingOptionId ? 2 :
              !termsAccepted ? 3 : 4;
            ...
            return (
              <li key={step} className="flex items-center">
                ...
                <span className={`ml-1.5 text-xs font-medium hidden sm:inline ${active ? "text-primary" : "text-on-surface-variant"}`}>
                  {step}
                </span>
                {i < arr.length - 1 && (
                  <span className="mx-2 h-px w-8 bg-outline-variant/40 sm:w-12" aria-hidden="true" />
                )}
              </li>
            );
          })}
      ...
      {isGuestCheckout && (
        <div className="mb-6 flex items-start gap-3 rounded-lg border border-outline-variant/20 bg-surface-container-low px-4 py-3 max-w-lg">
          <svg className="mt-0.5 h-4 w-4 shrink-0 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <div className="min-w-0">
            <p className="text-sm font-medium text-on-surface">Continuing as guest</p>
            <p className="mt-0.5 text-xs text-on-surface-variant">
              Enter your email below for order updates. Cash on delivery requires a{" "}
      ...
      <p className="font-body text-on-surface-variant mb-4 max-w-lg">
        Review your bag and choose how to pay. The total on the right includes shipping and taxes so you know exactly what you pay before clicking the button.
      </p>
      <p className="font-body text-sm text-on-surface-variant mb-12 max-w-lg rounded-lg border border-outline-variant/15 bg-surface-container-low/40 px-4 py-3">
        ...
        <span className="mt-2 block leading-relaxed">
          Your bag total loads with shipping and taxes included. For card payments, you continue to hosted Stripe Checkout. For PayPal, GCash, or PayMaya, you are sent to that provider. Cash on delivery places your order with no card step. Your saved delivery address is used for fulfillment.
        </span>
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="space-y-8">
                ...
                {medusaPricePreview.shippingOptions.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        !foreignCheckoutActive && setSelectedShippingOptionId(opt.id)
                      }
            ...
            >
              {(
                paymentAvailabilitySource === "medusa"
                  ? (Object.keys(PAYMENT_PROVIDER_IDS) as PaymentProviderKey[]).filter(
                      (k) => providerAvailable[k],
                    )
          ...
          <section>
            ...
            <div className="mt-6 border-t border-outline-variant/15 pt-5">
              ...
              {promoApplied ? (
                <div className="flex items-center gap-3 rounded bg-primary/10 px-4 py-3">
                  ...
                  <span className="flex-1 text-sm font-medium text-primary">
                    ...
                    {promoApplied.discountAmount != null && promoApplied.discountAmount > 0
                      ? ` (-${formatCheckoutMoney(promoApplied.discountAmount, displayCurrency)})`
                      : ""}
                  </span>
                  <button
                    type="button"
                    onClick={() => void removePromoCode(medusaPricePreview?.cartId ?? "")}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") void applyPromoCode(medusaPricePreview?.cartId ?? "");
                    }}
                  ...
                  <button
                    type="button"
                    onClick={() => void applyPromoCode(medusaPricePreview?.cartId ?? "")}
                    disabled={!promoCode.trim() || promoLoading || foreignCheckoutActive || !medusaPricePreview?.cartId}
              ...
              {promoError && (
                <p className="mt-2 text-xs text-error" role="alert">{promoError}</p>
              )}
              {!medusaPricePreview?.cartId && !promoApplied && (
                <p className="mt-1 text-xs text-on-surface-variant/60">
                  A Medusa cart must be initialised before a code can be applied. The code field is active once your bag totals load.
                </p>
              )}
            </div>
          </section>
        ...
        <div>
          <div className="bg-surface-container-lowest rounded-lg shadow-[0px_20px_40px_rgba(0,0,0,0.02)] p-6 border border-outline-variant/10">
                ...
                {lines.map((l) => (
                  ...
                  >
                    ...
                    <div className="flex flex-1 justify-between gap-2 min-w-0">
                      <div className="min-w-0">
                        <p className="font-medium text-primary truncate">{l.name}</p>
                        <p className="text-on-surface-variant text-xs mt-0.5">
                          {[l.size, l.color].filter((v) => v && v.trim() !== "" && !/^[\u2014\u2013\-]+$/.test(v.trim())).join(" / ") || "One size"}
                          {l.sku ? (
            ...
            {useMedusaBagTotals && medusaPricePreview ? (
                ...
                <>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-on-surface-variant italic">Calculating...</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant">
                      Tax
                  ...
                  <div className="flex justify-between font-headline font-bold text-lg pt-3 mt-1 border-t border-outline-variant/20">
                    <span>You pay</span>
                    <span className="text-on-surface-variant italic text-base font-normal">Calculating...</span>
                  </div>
                ...
                <>
                  <div className="rounded-md bg-amber-50 border border-amber-200 p-3 mb-2" role="status">
                    <p className="text-xs text-amber-900">
                      We could not load confirmed pricing. The estimate below does not include shipping.
                      Your final total will be confirmed when you continue to payment.
                    </p>
                    {medusaPreviewError ? (
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant">Subtotal (estimate)</span>
                    <span>
                      {formatCheckoutMoney(localSubtotal, displayCurrency)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant">Shipping</span>
                    <span className="text-on-surface-variant italic text-xs">Added at payment</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-on-surface-variant">
                  ...
                  <p className="text-xs text-amber-800 pt-1 leading-relaxed">
                    Shipping will be added. Your final amount is confirmed before payment.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-xs text-on-surface-variant pt-1 leading-relaxed">
                    Shipping and final tax will be added. Your exact amount is confirmed before you pay.
                  </p>
                </>
              )}
            </div>
            ...
            {pendingPayment && (
              ...
              >
                ...
                {pendingPayment.priceMismatch ? (
                  <div className="rounded bg-amber-50 border border-amber-200 px-3 py-2 mb-3" role="status">
                    <p className="text-xs text-amber-900">
                      The final amount differs from the earlier estimate because shipping or taxes were updated.
                      The amount shown above is what you will pay.
                    </p>
                  </div>
                ) : null}
                <p className="text-sm text-on-surface-variant mb-3">
                  Open {pendingPayment.providerLabel} in a secure tab, complete payment there, then return
                  here to confirm the result if the provider does not bring you back automatically.
                  Save your tracking link to follow your order status.
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    ...
                    onClick={() => void resumePendingHostedPayment(pendingPayment)}
            ...
            <div className="mt-6">
              <label htmlFor="delivery-instructions" className="mb-1 block text-xs font-bold uppercase tracking-wider text-on-surface-variant">
                Special delivery instructions <span className="font-normal normal-case">(optional)</span>
              </label>
              <textarea
