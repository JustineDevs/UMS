...
function checkGitignore() {
  ...
  const hasEnv =
    /^\.env\.local$/m.test(content) || /^\.env\.local\s*$/m.test(content);
...
function main() {
  ...
  if (!gitignore.exists) {
    ...
    log("⚠️  .gitignore may not properly exclude .env.local files", "yellow");
    log("   Ensure .gitignore contains: .env.local", "yellow");
  } else {
    log("✅ .gitignore properly configured for .env.local files", "green");
  }
  ...
  if (sensitiveStaged.length > 0) {
    log(
      `❌ Found ${sensitiveStaged.length} sensitive file(s) in staging area:`,
      "red",
    );
  ...
  if (sensitiveWorking.length > 0) {
    log(
      `⚠️  Found ${sensitiveWorking.length} sensitive file(s) in working directory:`,
      "yellow",
    );
  ...
  log(
    `   Sensitive files found: ${sensitiveStaged.length + sensitiveWorking.length}`,
