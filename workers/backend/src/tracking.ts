...
export async function resolveTrackingCapability(
  token: string,
  env: TrackingEnv,
  ...
  try {
    ...
    if (
      ...
      !["track", "confirmation"].includes(payload.purpose ?? "") ||
      payload.audience !== "public-tracking" ||
      payload.keyVersion !== keyVersion ||
      payload.issuedAt !== issuedAt ||
      payload.expiresAt !== expiresAt ||
      typeof payload.id !== "string" ||
      !/^order_[A-Za-z0-9_-]+$/.test(payload.id)
    )
      return null;
    const scope =
      payload.scope && typeof payload.scope === "object"
        ? {
            ...(typeof payload.scope.customerEmailHash === "string" &&
            /^[a-f0-9]{64}$/.test(payload.scope.customerEmailHash)
              ? { customerEmailHash: payload.scope.customerEmailHash }
              : {}),
            ...(typeof payload.scope.storeId === "string" &&
            payload.scope.storeId.trim()
...
function status(row: OrderRow): string {
  ...
  const events = Array.isArray(metadata.pancake_pos_events)
    ? metadata.pancake_pos_events
    : Array.isArray(metadata.shipment_events)
      ? metadata.shipment_events
      : [];
  const latest = events
  ...
  if (
    ["out_for_delivery", "in_transit", "shipped"].includes(
  ...
  if (
    ...
    metadata.cod_payment_status !== "captured" &&
...
function shipments(row: OrderRow): Array<Record<string, unknown>> {
  ...
  const source =
    ...
    metadata.pancake_pos_shipments.length > 0
      ...
      : Array.isArray(metadata.fulfillment_shipments)
        ? metadata.fulfillment_shipments
        : [];
  return source.flatMap((value, index) => {
    ...
    const tracking =
      typeof shipment.tracking_number === "string"
        ? shipment.tracking_number.trim().slice(0, 80)
        : undefined;
    return [
      {
...
export async function handleTrackingRequest(
  ...
  const result = await database.query<OrderRow>(
    `SELECT o.id, o.display_id, o.updated_at,
            ...
            0::numeric AS tax_total, 0::numeric AS shipping_total, 0::numeric AS discount_total,
            (SELECT pc.status
  ...
  if (capability.scope?.customerEmailHash) {
    if (
      !row.email ||
      (await sha256Hex(row.email.trim().toLowerCase())) !==
  ...
  if (capability.scope?.storeId) {
    const storeId =
      typeof metadata.store_id === "string"
        ? metadata.store_id
        : typeof metadata.organization_id === "string"
          ? metadata.organization_id
          : null;
    if (storeId !== capability.scope.storeId)
      return json({ error: "not_found" }, 404);
  }
