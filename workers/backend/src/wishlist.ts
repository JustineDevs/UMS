...
function productId(value: unknown): string | null {
  return typeof value === "string" &&
    /^[A-Za-z0-9_-]{1,200}$/.test(value.trim())
    ? value.trim()
    : null;
}
...
async function canonicalItems(
  app: WorkerDatabaseClient,
  medusa: WorkerDatabaseClient,
  id: string,
): Promise<unknown[]> {
  ...
  for (const row of rows.rows) {
    const product =
      typeof row.medusa_product_id === "string"
        ? await canonicalProduct(medusa, row.medusa_product_id)
        : null;
    if (!product) continue;
    items.push({
...
async function handleGet(
  ...
  return withDb(env, "medusa", async (medusa) => {
    ...
    return json({
      items: await withDb(env, "app", (app) => canonicalItems(app, medusa, id)),
    });
  });
}
...
export async function handleWishlistRequest(
          ...
          .map((item) =>
            productId(
              item && typeof item === "object"
                ? (item as Record<string, unknown>).medusaProductId
                : null,
            ),
          )
  ...
  return withDb(env, "medusa", async (medusa) => {
    ...
    const products = (
      await Promise.all(
        productIds.map((value) => canonicalProduct(medusa, value)),
      )
    ).filter((value): value is Row => Boolean(value));
    const skippedProductIds = productIds.filter(
