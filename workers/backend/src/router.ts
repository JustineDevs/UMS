...
export async function handleBackendRequest(
  ...
  if (request.method === "GET" && (path === "/healthz" || path === "/readyz")) {
    const ready = path === "/healthz" || hasConfiguredDatabase(env);
  ...
  const socialProofMatch = request.method === "GET" && path === "/store/social-proof";
  ...
  const adminCatalogProductsMatch =
    ...
    (path === "/api/admin/catalog/products" || path === "/admin/catalog/products");
  const adminCatalogCategoriesMatch =
    ["GET", "POST"].includes(request.method) &&
    (path === "/api/admin/catalog/categories" || path === "/admin/catalog/categories");
  const adminCatalogProductMatch =
  ...
  const cmsAdminMediaListMatch =
    request.method === "GET" &&
    (path === "/admin/cms/media" || path === "/api/admin/cms/media");
  const cmsAdminMediaUploadMatch =
    request.method === "POST" &&
    (path === "/admin/catalog/media" || path === "/api/admin/catalog/media");
  const cmsAdminMediaDetailMatch =
  ...
  if (
    ...
    try {
      const nativeResponse = await withWorkerDatabase(
        env as BackendEnv & WorkerDatabaseEnv,
        (database) => handleCatalogProviderSyncRequest(request, database, env),
        "app",
      );
  ...
  if (
    (adminOrdersMatch || adminOrderDetailMatch || adminInventoryMatch || adminCatalogProductsMatch || adminCatalogProductMatch || adminCatalogCategoriesMatch || adminCustomersMatch) &&
    hasConfiguredDatabaseForRole(env, "medusa")
  ) {
    try {
      const nativeResponse = await withWorkerDatabase(
        ...
        (database) =>
          adminCatalogCategoriesMatch
            ...
            : adminOrderDetailMatch
            ? handleAdminOrderDetailRequest(request, database, env, decodeURIComponent(adminOrderDetailMatch[1]))
            : adminOrdersMatch
            ...
            : adminInventoryMatch
              ...
              : adminCatalogProductsMatch && request.method === "POST"
                ? handleAdminCatalogProductMutationRequest(request, database, env)
              : adminCatalogProductMatch && request.method === "PATCH"
                ? handleAdminCatalogProductMutationRequest(request, database, env, decodeURIComponent(adminCatalogProductMatch[1]))
              : adminCatalogProductMatch && request.method === "DELETE"
                ? handleAdminCatalogProductDeleteRequest(request, database, env, decodeURIComponent(adminCatalogProductMatch[1]))
              : adminCatalogProductMatch
                ? handleAdminCatalogProductRequest(request, database, env, decodeURIComponent(adminCatalogProductMatch[1]))
              : adminCatalogProductsMatch
                ? handleAdminCatalogProductsRequest(request, database, env)
                : handleAdminCustomersRequest(
                    request,
                    database,
                    env,
  ...
  if (deliveryShipmentsMatch && hasConfiguredDatabaseForRole(env, "app")) {
    try {
      const nativeResponse = await withWorkerDatabase(
        env as BackendEnv & WorkerDatabaseEnv,
        (database) => handleDeliveryShipmentsRequest(request, database, env),
        "app",
      );
      const headers = new Headers(nativeResponse.headers);
      headers.set("X-Request-ID", id);
                        ...
                        : cmsAdminCreateMatch
                          ...
                          : cmsAdminUpdateMatch
                            ? handleCmsAdminPageRequest(
                                ...
                                database,
                                env,
                                decodeURIComponent(cmsAdminUpdateMatch[1]),
                              )
                              : socialProofMatch
                                ? handleSocialProofRequest(request, database)
                                : productMatch
                              ? handleCatalogProductRequest(
                                  request,
                                  database,
